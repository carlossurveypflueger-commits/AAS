...
Seja criativo e pense em como REALMENTE encontrar fotos deste produto."""
    ...
    def _guess_usage_context(self, produto: str, tipo_produto: str) -> List[str]:
        ...
        if any(word in produto_lower for word in ['phone', 'celular', 'smartphone']):
            contexts.extend(["person using phone", "phone in hand"])
    ...
    async def _execute_universal_search(self, strategy: Dict) -> List[Dict]:
        ...
        for term in search_terms[:2]:  # Máximo 2 termos por estratégia
            logger.info(f"[{strategy_name.upper()}] Buscando: '{term}'")
    ...
    async def _search_pexels_universal(self, term: str) -> List[Dict]:
        ...
        try:
            async with httpx.AsyncClient() as client:
                response = await client.get(
                    ...
                    params={
                        "query": term,
                        "per_page": 15,
                        "orientation": "all",  # Todas orientações
                        "size": "medium",
                        "locale": "en-US"
                    },
                    timeout=10
            ...
            if response.status_code == 200:
                ...
                refs = []
                for photo in photos[:10]:  # Limitar para performance
                    refs.append({
                        "url": photo["src"]["large"],
